...
**e2e-testing** gives agents a fresh Docker sandbox for repo-level end-to-end verification when they need evidence from a real running environment instead of local-only checks. The skill itself encodes the goal-selection, env-discovery, mock-vs-prompt, and evidence heuristics, while deterministic helper scripts only scaffold the output directory and artifact files. The result is still the same structured artifact set (`report.md`, `report.json`, `demo.md`, command logs), but without hiding a second prompt system inside a vendored runtime.
